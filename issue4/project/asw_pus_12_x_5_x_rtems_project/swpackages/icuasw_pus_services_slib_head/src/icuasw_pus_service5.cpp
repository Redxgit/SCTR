...
void PUSService5::Exec5_5TC( CDTCDescriptor &TC, CDTMList &List){
  ...
  if (GetRIDEnableIndex(RID, index)){
	  printf("PCUSTOM PRINT:: Previous value for RID %d with index %d: %d\n",RID,index, RIDEnableConfig[RID].RID);
	  RIDEnableConfig[RID].RID=true;
	  printf("CUSTOM PRINT:: Current value for RID %d with index %d: %d\n",RID,index, RIDEnableConfig[RID].RID);
	  List.Add(1,7);
  }
  else{
	  List.Add(1,8);
...
void PUSService5::Exec5_6TC( CDTCDescriptor &TC, CDTMList &List){
	  ...
	  if (GetRIDEnableIndex(RID, index)){
		  printf("CUSTOM PRINT:: Previous value for RID %d with index %d: %d\n",RID,index, RIDEnableConfig[RID].RID);
		  RIDEnableConfig[RID].RID=false;
		  printf("CUSTOM PRINT:: Current value for RID %d with index %d: %d\n",RID,index, RIDEnableConfig[RID].RID);
		  List.Add(1,7);
	  }
	  else{
